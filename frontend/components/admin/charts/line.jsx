...
function madeBeforeDate(date, comparisonDate) {
...
export default class LineChartElement extends Component {
	state = {
		changeColour: false,
		colourAccent: "#1a85ff",
		colourBlack: "#141213",
	...
	render() {
		...
		for (var i = 1; i <= this.state.days; i++) {
			...
			const dataCount = data.filter(dataItem =>
				madeBeforeDate(new Date(dataItem.createdAt), date)
			).length;
			days[dateFormatted] = dataCount;
		}

		days[moment(today).format("DD/MM/YYYY")] = data.length;
